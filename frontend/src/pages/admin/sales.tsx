...
export default function AdminSales() {
  ...
  // On mobile the invoice lives behind a small toggle button so the product
  // grid stays the main, fully usable surface. On desktop it's always visible.
    ...
    <>
      <div className="border-b border-[#e4d9d0] bg-[#fbf7f1] px-5 py-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black text-[#33251f]">الفاتورة الحالية</h2>
            <p className="mt-1 text-xs text-[#927d70]">{itemCount} قطعة</p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-5" data-testid="cart-lines">
        {cart.length ? (
          <div className="divide-y divide-[#eee6df]">
            {cart.map((line) => (
              <div key={line.id} className="py-5" data-testid={`row-cart-product-${line.id}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-black text-[#352721]">{line.name}</p>
          ...
          >
            ...
            <p className="mt-4 font-black text-[#594239]">الفاتورة فارغة</p>
            <p className="mt-1 text-xs text-[#9b887d]">اختر منتجاً لإضافته إلى الفاتورة</p>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-[#dfd4cc] bg-[#fffdf9] px-5 py-5">
        ...
        <div className="mt-4 border-t border-dashed border-[#d8cbc1] pt-4">
          <div className="flex items-center justify-between">
            <span className="text-base font-black text-[#46342c]">الإجمالي</span>
            <strong
        ...
        <div className="mt-5">
          <p className="mb-2 text-xs font-black text-[#684a3e]">طريقة الدفع</p>
        ...
        {paymentMethod === "debt" && (
          ...
          >
            <div>
              <label className="mb-1.5 block text-xs font-bold text-[#70483b]" htmlFor="customer-select">
                الزبون
              </label>
            ...
            <div>
              <label className="mb-1.5 block text-xs font-bold text-[#70483b]" htmlFor="paid-amount">
                المدفوع الآن
              </label>
            ...
            <div className="flex items-center justify-between border-t border-[#ecd9cf] pt-3">
              <span className="text-sm font-bold text-[#815446]">المتبقي</span>
              <strong className="text-[#a34f3c]" data-testid="text-remaining-amount">
        ...
        <p className="mt-3 text-center text-[10px] text-[#a18c81]">المبالغ محسوبة فورياً</p>
      </div>
    </>
  );

  return (
    <div dir="rtl" className="text-[#30231e]" data-testid="page-admin-sales">
      {/* PAGE HEADER — sits inside AdminLayout's <main>, no duplicate branding/menu here */}
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#713a24] text-white">
            <Receipt className="h-5 w-5" />
          </div>
          <h1 className="text-2xl font-black text-[#33251f]">نقطة البيع</h1>
        </div>

        <button
          type="button"
          onClick={startNewSale}
          className="hidden items-center gap-2 rounded-xl border border-[#d7c8ba] bg-white px-4 py-2 text-sm font-bold text-[#74462f] hover:bg-[#f1e7dc] sm:flex"
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <div className="min-w-0">
          ...
          <div className="mb-4 flex items-end justify-between">
            <div>
              <p className="text-xs font-bold tracking-[0.18em] text-[#9b7663]">كتالوج المتجر</p>
              <h2 className="mt-1 text-2xl font-black text-[#33251f]">اختر المنتجات</h2>
            </div>
          ...
          {filteredProducts.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="px-3 pb-3">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-[#927e72]">متوفر {product.stock}</span>
                      </div>
                    </div>
                  </button>
                );
            ...
            >
              ...
              <p className="mt-3 font-black text-[#594239]">لا توجد منتجات</p>
              <button
                type="button"
                onClick={() => {
                  setSearch("");
        ...
        {/* INVOICE — inline column on desktop, slide-in drawer on mobile */}
        <aside className="hidden flex-col overflow-hidden rounded-2xl border border-[#e4d9d0] bg-white shadow-[0_4px_20px_rgba(65,40,25,0.06)] lg:sticky lg:top-6 lg:flex lg:h-[calc(100vh-3rem)]">
          {invoicePanel}
        </aside>
      </div>
      ...
      <button
        ...
        className="fixed bottom-6 left-6 z-40 flex items-center gap-2 rounded-full bg-[#713a24] px-5 py-3.5 text-white shadow-[0_8px_20px_rgba(113,58,36,0.35)] lg:hidden"
      ...
      <aside
        className={`fixed bottom-0 top-0 right-0 z-50 flex w-[88%] max-w-[380px] flex-col bg-white shadow-[-5px_0_20px_rgba(65,40,25,0.08)] transition-transform duration-300 lg:hidden ${
